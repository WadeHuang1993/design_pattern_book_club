...
面向物件設計最困難的部分是將系統分解成物件集合，因為要考慮許多因素：
...
設計模式幫你定並不明顯的抽象和述這些抽象的物件例如，
描述過程或算法的物件現實中並不存在但它們卻設計的關鍵部分。

Strategy 模式描述了怎樣實現可互換的演算法。
State 模式將實體的每個狀態描述為一個物件。
...
1. 因為繼承在編譯時期就定義了，所以無法在運行時刻改變從類繼承的實現內容
2. 子類中的實現與它的父類有緊密的依賴關係，以至於父類實現中的任何變化必然會導致子類發生變化。
3. 繼承下來的實現不適合解決新的問題，則父類必須重寫或被其他更適合的類替換。這種依賴關係限制了靈活性


一般的物件組合技術和具體的委託技術，是繼承之外組合物件行為的另一種靈活方法。
新的功能可以通過以新的方式組合現有物件，而不通過定義已存在類的子類的方式加到應用中去。

另一方面，過多使用物件組合會使設計難於理解。許多設計模式產生的設計中，你
可以定義一個子類，且將它的實例和已存在實例進行組合來引人定製的功能。
